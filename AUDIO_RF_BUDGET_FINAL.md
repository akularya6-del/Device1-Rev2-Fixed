...
| **Total Packet Size** | **80 bytes** | **640 bits total over-the-air** |
...
- **Packet Airtime ($T_{packet}$):**
  $$T_{packet} = 640\text{ bits} \times 6.667\ \mu\text{s/bit} = 4.267\text{ ms}$$
- **PA Ramp-up / Settling:** $40\ \mu\text{s}$ ($0.040\text{ ms}$).
- **Total TX Slot Duration:** $4.307\text{ ms}$.
- **RX Listening Window:** $0.500\text{ ms}$ (500 µs ACK listening slot).
- **Radio State Turnaround Margin:** $0.150\text{ ms}$ (150 µs PLL lock and mode transitions).
- **Total Active Window:** $4.307 + 0.500 + 0.150 = 4.957\text{ ms}$.
- **Audio Frame Interval:** $8.000\text{ ms}$ ($128\text{ samples} / 16,000\text{ Hz}$).
- **MCU Processing & Idle Margin:**
  $$\text{Idle Margin} = 8.000\text{ ms} - 4.957\text{ ms} = 3.043\text{ ms}\quad (\mathbf{38.04\%\ margin})$$
- **Duty Cycle:** $4.307\text{ ms} / 8.000\text{ ms} = 53.84\%$.
...
This calculation supports the design target of **~400–500 meters line-of-sight** with a theoretical link margin of $+4.5\,\text{dB}$. Actual field range and multipath performance require physical validation in real-world deployment environments.
