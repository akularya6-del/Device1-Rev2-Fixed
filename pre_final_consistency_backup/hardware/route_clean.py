...
def build():
    ...
    # 3. Footprints Placement (Courtyards > 0.35mm, distance from edge > 0.8mm)
    fp_defs = [
    ...
    # 3V3 Power Plane Drop Vias (in clear corridors)
    # Via at U5 VOUT (LDO)
    add_track(4.0, 19.85, 4.8, 19.85, pcbnew.F_Cu, p3v3_net, 0.30)
    add_via(4.8, 19.85, p3v3_net)
    # Connect C21 to 3V3
    add_track(9.0, 21.28, 8.0, 21.28, pcbnew.F_Cu, p3v3_net, 0.30)
    add_via(8.0, 21.28, p3v3_net)
    # Connect MK1 3V3
    add_track(8.4, 27.436, 9.2, 27.436, pcbnew.F_Cu, p3v3_net, 0.25)
    add_via(9.2, 27.436, p3v3_net)
    # Connect TP_3V3 on B.Cu
    add_via(11.5, 12.0, p3v3_net)
    # Connect U1 3V3 pins via 3V3 plane vias
    # Left channel: C1, C7, C11
    add_track(2.0, 10.48, 2.0, 11.2, pcbnew.F_Cu, p3v3_net, 0.25)
    add_via(2.0, 11.2, p3v3_net)
    add_track(2.0, 15.48, 2.0, 16.2, pcbnew.F_Cu, p3v3_net, 0.25)
    add_via(2.0, 16.2, p3v3_net)
    # Right channel: C2, C3, C4, C5
    add_track(13.0, 5.98, 12.0, 5.98, pcbnew.F_Cu, p3v3_net, 0.25)
    add_via(12.0, 5.98, p3v3_net)
    add_track(13.0, 10.98, 12.0, 10.98, pcbnew.F_Cu, p3v3_net, 0.25)
    add_via(12.0, 10.98, p3v3_net)
    add_track(13.0, 13.48, 12.0, 13.48, pcbnew.F_Cu, p3v3_net, 0.25)
    add_via(12.0, 13.48, p3v3_net)
    # U1 3V3 power pins to internal plane
    # Pins 37, 41, 44, 45, 46 on bottom edge (Y=15.938)
    for px in [4.75, 6.75, 8.25, 8.75, 9.25]:
        add_track(px, 15.938, px, 16.6, pcbnew.F_Cu, p3v3_net, 0.20)
        add_via(px, 16.6, p3v3_net)
    # Pins 25, 28, 29 on left edge
    for py in [9.75, 11.25, 11.75]:
        add_track(4.062, py, 3.4, py, pcbnew.F_Cu, p3v3_net, 0.20)
        add_via(3.4, py, p3v3_net)
    # Pin 11 on right edge
    add_track(10.938, 10.25, 11.6, 10.25, pcbnew.F_Cu, p3v3_net, 0.20)
    add_via(11.6, 10.25, p3v3_net)

    # RF Matching & 50-Ohm CPWG line
    add_track(5.25, 9.062, 5.25, 7.285, pcbnew.F_Cu, nets["RF_50OHM"], 0.29)
    add_track(5.25, 7.285, 3.02, 6.8, pcbnew.F_Cu, nets["RF_50OHM"], 0.25)
    ...
    board.Save('/Users/racoon/Documents/CHEATING/Device1/hardware/test_clean.kicad_pcb')
