#!/usr/bin/env python3
"""
Refine Device 1 Rev 2.0 PCB to achieve 0 DRC Violations.
"""
...
def build_board():
    ...
    route_points([(3.0, 13.5), (0.5, 13.5), (0.5, 26.2), (6.6, 26.2)], pcbnew.B_Cu, mic_ws_net, 0.15)

    # MIC_SD (B.Cu at X=0.9, arrives at Y=26.8)
    mic_sd_net = nets["MIC_SD"]
    u1_33 = get_pad_pos("U1", "33") # (4.062, 14.25)
    mk1_6 = get_pad_pos("MK1", "6") # (7.500, 27.136)
    add_via(2.2, 14.5, mic_sd_net)
    route_points([u1_33, (2.2, 14.25), (2.2, 14.5)], pcbnew.F_Cu, mic_sd_net, 0.15)
    add_via(7.5, 26.8, mic_sd_net)
    add_track(7.5, 26.8, mk1_6[0], mk1_6[1], pcbnew.F_Cu, mic_sd_net, 0.15)
    route_points([(2.2, 14.5), (0.9, 14.5), (0.9, 26.8), (7.5, 26.8)], pcbnew.B_Cu, mic_sd_net, 0.15)
    ...
    route_points([(8.25, 8.5), (8.25, 27.2), (9.2, 27.2), (9.2, 27.958)], pcbnew.B_Cu, mic_sck_net, 0.15)
    ...
    # LDO Output: U5.5 -> C20.1 -> C21.1 -> via to In2.Cu plane at (5.0, 17.5)
    ...
    add_via(8.4, 26.0, p3v3_net)
    add_track(mk1_5[0], mk1_5[1], 8.4, 26.0, pcbnew.F_Cu, p3v3_net, 0.25)
    ...
    board.Save(out_pcb)
    print(f"Refined board saved to {out_pcb}")
